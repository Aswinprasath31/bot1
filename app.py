...
# Config / paths
# -------------------------
DATA_PATH = "/mnt/data/training_data.csv"  # your uploaded CSV
...
def load_qa_dataframe(path=DATA_PATH):
    df = pd.read_csv(path)
    # split column like "Question: ... Answer: ..."
...
# Keyword / similarity-based evaluation using TF-IDF + cosine
...
# Very small rubric: presence of essential keywords (optional)
def keyword_overlap_score(expected, user, top_n=8):
    # take top terms from expected (naive approach)
    expected_norm = normalize_text(expected)
    tokens = expected_norm.split()
    # remove common words (very small stoplist)
    stop = set(["the","a","an","and","is","are","of","in","to","for","with","that","this"])
    tokens = [t for t in tokens if t not in stop]
    key_terms = list(dict.fromkeys(tokens))[:top_n]  # unique order-preserving
    user_norm = normalize_text(user)
...
st.markdown(
    "Practice Q&A from your dataset. This prototype evaluates answers using TF-IDF similarity and keyword overlap. "
)
...
if mode == "Random questions":
    if st.button("Start new random session"):
        st.session_state.remaining_qs = random.sample(list(qa.index), k=min(num_questions, len(qa)))
elif mode == "Topic search":
    topic = st.sidebar.text_input("Enter a topic keyword (e.g., 'neural', 'regression')")
    ...
    # Show a searchable list and let user pick
...
# If there are queued questions, pop the first and present
if st.session_state.remaining_qs:
    q_idx = st.session_state.remaining_qs.pop(0)
    q_row = qa.loc[q_idx]
    st.subheader("Question")
    st.write(q_row["question"])
    user_answer = st.text_area("Your answer (type below)", height=180)
    ...
    with col1:
        if st.button("Submit Answer"):
            if not user_answer.strip():
                ...
                save_response(conn, q_row["question"], q_row["answer"], user_answer, score, method=method_name)
                st.success(f"Score: {score:.1f} / 100")
                st.info("Expected answer (for learning):")
    ...
    with col3:
        if st.button("Skip question"):
            st.info("Question skipped — no score saved.")

else:
    st.info("No questions queued. Use the sidebar to start a session or add questions.")

# Show recent attempts
if st.checkbox("Show recent attempts (last 20)"):
...
if st.button("Export logs to CSV"):
    ...
    st.success(f"Saved {len(df_logs_all)} rows to `{csv_path}`. Download it from your environment.")

st.markdown("---")
st.markdown("### Tips")
st.markdown(
    "- Use **Random** mode for quick mocks.\n"
    "- Use **Topic search** to focus on subjects (e.g., 'overfitting', 'PCA').\n"
    "- Scores are heuristic — combine with human review for final feedback.\n"
)
